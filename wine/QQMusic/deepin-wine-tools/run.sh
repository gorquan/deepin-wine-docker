...
CallWangWang() {
    if [ "autostart" == "$1" ]; then
        env WINEPREFIX="$WINEPREFIX" $WINE_CMD /opt/deepinwine/tools/sWINEPREFIX="$WINEPREFIX" deepin-wine "c:\\Program Files\\Tencent\\QQMusic\\QQMusic.exe" &tartbottle.exe &
    else
...
CallQQMusic() {
    if [ "autostart" == "$1" ]; then
        ...
        env WINEPREFIX="$WINEPREFIX" deepin-wine "c:\\Program Files\\Tencent\\QQMusic\\QQMusic.exe" &
    fi
}
